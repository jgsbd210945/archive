...
The following are given; This sets up the data and does preliminary checks on happiness by country. We also use an intervening variable - belief in God - to see correlation.
...
Before doing anything else, I'll clean the data of NAs.
...
### Variable Cleaning/Correlation

Here, I'll run the 12 explanatory variables as requested, cleaning the data and doing some exploratory analysis. In addition to the 7 given, I'll be doing correlations on first- and second-generation immigrant status, closeness to country and nationalism more generally, and satisfaction with political system.

I'll start with the 7 given. Since we already did country (by mean happiness - not much we can do for correlations there), I'll be excluding that one.

```{r eda1}
## Gender
wvs$gender = ifelse(wvs$Q165>0, wvs$Q165-1, NA) # 0 is male, 1 is female
sum(is.na(wvs$gender))
wvs |> 
  group_by(country) |>
...
Now, I'll do correlations on immigrant status - both if they are an immigrant, and if either or both of their parents are immigrants.
...
I'll also do how closeness to country affects happiness alongside national pride. I reverse the pattern here, so larger numbers indicate increased closeness.
...
wvs |> 
  ...
  geom_hline(yintercept = 0, linetype = "dashed", col='darkred') +
  coord_flip() +
  ggtitle('Where nationalism is most positively correlated with happiness') +
  ylab('correlation between nationalism and happiness')
```

Finally, I'll do satisfaction in political system performance.
...
I should have already cleaned a few variables here, but I'll clean 5 more to add to the 5 new variables I already cleaned. Those will be status of democracy, importance of democracy, belief in civil rights, left-right scale, and belief in having a strong leader.
...
### Running OLS and Poly

Since the assignment doesn't specify what to run OLS *on*, I did a multiple linear regression with all of the variables I've cleaned, and then ran it on a few select variables that had high correlations.
...
For polynomial regression, I'll be using `age` as required.
...
poly3 <- lm(happiness ~ poly(age, 3), data = wvs_filteredage)
summary(poly3)
```

As we can see, the second-degree polynomial has the least RSE (tied with the third-degree), but has the highest adjusted R-squared, making it the best fit.

### Ordered Logit Regression - `polr`

We can also do a logit model for this. However, we'll need to factorize happiness in order to make it work properly.
...
### Predicting using an Ordinal Logistic Model

Finally, we will create a new OLM to test age *and* country, and generate fake data with which we can test it with.

After making the test model, we then add in all the different operators we've tested previously, using that data to predict the probability the respondent will respond to different levels of happiness.

We've also plotted these in a scatter plot to display the predicted chance of happiness based on age and country.
